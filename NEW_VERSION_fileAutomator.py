...
dest_dir_documents = os.path.join(base_dir, "Documents")
dest_dir_installation = os.path.join(base_dir, "Installtions")
dest_dir_compressed = os.path.join(base_dir, "Compressed")
dest_dir_other = os.path.join(base_dir,  "Other")
...
def move_file(dest, entry, name):
    ...
    for _ in range(3):  # Retry the operation up to 3 times
        try:
        ...
        sleep(1)  # Introduce a delay between attempts

    else:
        logging.error(f"Failed to move file {name} after multiple attempts.")
